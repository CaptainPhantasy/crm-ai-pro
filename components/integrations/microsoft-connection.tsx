...
import { Mail, CheckCircle2, RefreshCw, Download, Users, MessageSquare } from 'lucide-react'
...
export function MicrosoftConnection() {
  ...
  const [providers, setProviders] = useState<MicrosoftProvider[]>([])
  ...
  async function fetchStatus() {
    try {
      ...
      console.error('Error fetching Microsoft status:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleConnect() {
    try {
      ...
      const response = await fetch('/api/integrations/microsoft/authorize')
      if (response.ok) {
        const data = await response.json()
        ...
        setError(errorData.error || 'Failed to initiate Microsoft connection')
      }
    } catch (error) {
      console.error('Error connecting Microsoft:', error)
      setError('Failed to connect Microsoft. Please try again.')
    } finally {
      setConnecting(false)
    }
  }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="mt-1">
              Connect your Microsoft 365 account to send emails directly from your business email
            </CardDescription>
          </div>
          {isConnected && (
        ...
        {isConnected ? (
          <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium text-sm">
                      {provider.provider_email || 'Microsoft 365 Account'}
                    </div>
            ...
            <div className="pt-4 border-t">
              ...
              <div className="text-xs text-neutral-500 mb-3">
                Sync emails from your Microsoft 365 account to automatically create contacts and conversations in your CRM.
              </div>
              <Button
                onClick={handleSync}
                disabled={syncing}
                className="w-full bg-[#0078D4] hover:bg-[#0064B4] text-white"
              >
                {syncing ? (
                  <>
          ...
          <div className="space-y-4">
            <div className="text-sm text-neutral-600">
              Connect your Microsoft 365 account to send emails directly from your business email address.
              This allows you to:
            </div>
            <ul className="text-sm text-neutral-600 space-y-1 list-disc list-inside">
              <li>Send emails from your business Microsoft 365 address</li>
              <li>Maintain email threading and conversations</li>
              <li>Use your company's email domain and branding</li>
              <li>Access Microsoft 365 sending infrastructure</li>
            </ul>
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-md text-xs text-blue-700">
              <strong>Note:</strong> This requires a Microsoft 365 business account. 
            ...
            <div className="text-xs text-neutral-500 text-center">
              You'll be redirected to Microsoft to authorize access to your account
            </div>
          </div>
        )}
      </CardContent>
