  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          "bg-theme-card data-[state=open]:animate-in data-[state=closed]:animate-out fixed z-50 flex flex-col gap-4 shadow-elevated transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 border-theme-border",
          side === "right" &&
      ...
      >
        {children}
        <SheetPrimitive.Close className="ring-offset-theme-card focus:ring-theme-accent-primary data-[state=open]:bg-theme-secondary absolute top-4 right-4 rounded-xs opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none text-theme-accent-primary">
...
function SheetDescription({
  ...
  return (
    <SheetPrimitive.Description
      ...
      className={cn("text-theme-subtle/70 text-sm", className)}
      {...props}
    />
  )
}
